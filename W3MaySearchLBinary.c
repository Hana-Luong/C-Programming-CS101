/* Page 274:  Exercises 6.32– and 6.33 ask you to implement
...
int main(void)
{
    ...
    size_t result = binarySearch(a, key, 0, SIZE - 1);//low = 0; size = SIZE -1
...
size_t binarySearch(const int b[], int searchKey, size_t low, size_t high)
{
    ...
    while (low <= high) 
    {
        ...
        printRow(b, low, middle, high); //VERY ELEGANT
...
void printRow(const int b[], size_t low, size_t mid, size_t high)
{
    ...
    for (size_t i = 0; i < SIZE; ++i) 
    {
        ...
        if (i < low || i > high) 
        {
            printf("%s", "    "); //COUNT THE EMPTY SPACE HERE. 
        } 
        else if (i == mid) 
        {   
